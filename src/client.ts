...
export function assertAskOnlyRuleset(
  rules: readonly { action: string; permission: string; pattern?: string }[],
  ...
  const hasAsk = rules.some((r) => r.action === "ask");
  if (!hasAsk) return;
  ...
  for (const r of rules) {
    if (r.action === "allow" && r.permission === "external_directory") continue;
...
export interface CreateSessionOpts {
  ...
  allowedTools?: readonly string[];
  /** Canonical directory roots generated by `resolveReadPaths`; the only accepted source of
   * a session-level `external_directory` allow. */
  readPathRoots?: readonly string[];
  timeoutMs?: number;
  signal?: AbortSignal;
}
...
export async function createSession(opts: CreateSessionOpts): Promise<SessionRef> {
  ...
  if (opts.permission !== undefined && opts.permission.length > 0) {
    ...
    assertAskOnlyRuleset(opts.permission, opts.allowedTools, opts.readPathRoots);
...
export interface AskViaAgentOpts {
   ...
   */
  permission?: readonly SessionPermissionRuleWire[];
  /** Forwarded to `createSession` so the wire guard can authenticate generated read rules. */
  readPathRoots?: readonly string[];
  /**
...
export async function askViaAgent(serve: ServeProvider, opts: AskViaAgentOpts): Promise<AskResult> {
  ...
  if (gated) assertAskOnlyRuleset(opts.permission!, opts.allowedTools, opts.readPathRoots);
        ...
        const created = await createSession({
          ...
          ...(gated
            ? {
                ...
                readPathRoots: opts.readPathRoots,
              }
            : {}),
          timeoutMs: shortMs,
          signal: opts.signal,
