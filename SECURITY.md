...
- **Preflight.** `npx modelguild doctor` checks tools, auth, the MCP registration, the policy, and the payload before you rely on the commands. The resolved-config permission proofs (`verify-guild-*.sh`) are run locally after an opencode bump.
