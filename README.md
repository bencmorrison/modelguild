...
- **If the bridge loses opencode's event stream, that timeout stops applying to anything it can't see.** Open requests are rejected on the spot and the result is marked `degraded` — but a request the model raises while the bridge is blind never reaches you, and it waits on the turn's own timeout (`GUILD_MESSAGE_TIMEOUT_MS`, 15 minutes by default) instead of the approval timeout. The tool doesn't run in the meantime, so what you get is a long pause, not something slipping past the prompt. A `degraded` run showing no requests means the gate stopped seeing, not that the model stopped asking.
