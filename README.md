...
- **If your "yes" doesn't get through, the result says so.** opencode refuses a reply to a request it has already settled — which is what a healthy race looks like when your terminal and your MCP client both answer, and also what a *broken* approval path would look like. So the bridge asks a second question: is that request still open? Still open means nobody's decision landed, and it shows up as `unsettled` on the result with a plain-language reason instead of as a stall you have to guess at. (The endpoint that delivers an *approval* is the one opencode has marked deprecated; when it goes, approvals will fail while rejections keep working, and that is the failure this exists to name.) It will not quietly convert your approval into a rejection to get the turn moving again.
