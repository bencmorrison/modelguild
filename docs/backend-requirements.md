...
**opencode** is a binary on PATH, spawned as `serve --port N --hostname H` with its cwd fixed at spawn (`src/lifecycle.ts`, `OpencodeLifecycle.#startOnce`); readiness is polling `GET /doc` until it answers (`src/lifecycle.ts`, `OpencodeLifecycle.#startOnce`); teardown is keyed on stdin EOF and MCP transport close, not on signals (`src/lifecycle.ts`, `OpencodeLifecycle.attachShutdownTriggers`); and because opencode fences a model's reads at that cwd, a second root means a second supervised child, pooled per root (`src/servepool.ts`, `ServePool`).

**Minimum for a substitute:** a way to start it, a way to know it is ready, and a way to stop it that leaves nothing running once Claude Code closes the transport. Plus a way to fix the directory the model's reads and edits resolve against — per child or per call, either is fine, but a backend that cannot be rooted at a caller-named directory cannot serve the `worktree` input at all. The fence itself is `git worktree list` membership (`src/worktree.ts`, `resolveWorktreeTarget`) and is backend-neutral; what must survive verbatim is the coupling C71 states — the root the model edits and the root the capture snapshots are one value, never two.

### 2. The model turn

**Used by** `guild_consult`, `guild_panel`, `guild_research`, `guild_delegate`.

**opencode** creates a session naming the agent, the model and any per-session permission ruleset (`POST /session`, `src/client.ts`, `createSession`), runs the turn with a POST that blocks until it ends (`src/client.ts`, `sendMessage`), answers a continuation's stored ruleset and directory from `GET /session/{id}` (`src/client.ts`, `fetchSession`), and deletes a session on request (`src/client.ts`, `deleteSession`). The per-turn timeout applies to that one POST and nothing else (`src/config.ts`, `resolveMessageTimeoutMs`).
...
**opencode** returns the full ordered history with each message's `info` and `parts` from `GET /session/{id}/message` — the only sanctioned capture source, because the message POST's own body omits tool parts (`src/client.ts`, `fetchHistory`). The turn is bounded backwards to the last caller `user` message (`src/client.ts`, `turnStartIndex`), the answer is the ordered text — or, when those join to blank, reasoning — parts of the last answering assistant message (`src/client.ts`, `finalAssistantText`), and the tool-call count is the turn's `tool` parts (`src/client.ts`, `turnToolCallCount`).
...
**opencode** serves one `GET /event` SSE subscription per child with no replay (`src/activity.ts`, `ServeEventBus`), normalized from a probed event set — tool parts, file edits, permission requests, session idle and error. Gating adds a per-session `ask` ruleset supplied at session creation (`src/client.ts`, `createSession`) which may only ever narrow (`src/client.ts`, `assertAskOnlyRuleset`), a list of still-open requests to recover what was raised while the stream was down (`src/client.ts`, `listPendingPermissions`), and separate endpoints to approve and to reject one request (`src/approve.ts`, `ApprovalBridge`).
...
**opencode** answers `GET /config/providers` with the authed provider configuration (`src/models.ts`, `opencodeModels`) — per provider, not per model, so a listed id can still be rejected at call time.
...
**opencode** holds its own credentials from a login the user performs once; `doctor` probes the binary (`src/cli.ts`, `runDoctor`) and reports what `opencode auth list` can see (`src/cli.ts`, `opencodeAuthState`), warning rather than failing because that probe has known blind spots.
...
**opencode** resolves an agent definition from the serve cwd's `.opencode/agent/` or the global opencode agent dir, whose presence is checked before the call (`src/config.ts`, `hardenedDefPresentIn`); `GET /agent` then returns every agent **as opencode resolved it**, with its permission rule array (`src/client.ts`, `listAgents`), and the effective action for a sentinel tool no definition grants must come back `deny` (`src/agentfloor.ts`, `effectiveAction`, probe name at `src/agentfloor.ts`, `FLOOR_PROBE`).
