...
**Dotfiles-managed config works.** If `~/.claude` or `${XDG_CONFIG_HOME:-~/.config}/opencode` is a symlink — GNU stow, chezmoi, a hand-rolled `ln -s` — `--global` follows it and the files land in your dotfiles backing store, where you can commit them; the link itself is left alone, and `--uninstall --global` removes them back out through it. **Individual payload files can be symlinked too** — which is the shape GNU stow actually produces most of the time, because it only folds a whole tree when the parent directory is empty, and `~/.claude` rarely is (it holds `settings.json`, `projects/`, credentials). chezmoi's `symlink_` targets are the same. `init --global` settles a symlinked payload file by the same **ownership** rule it applies to every other file, from the bytes rather than the path type: it reads *through* the link, and if what it finds is what `init` last wrote there, it writes through the link too — so your dotfiles copy gets the upgrade and the link stays a link. If the bytes are something else (you edited it, or it was never ours), it is skipped and left untouched exactly as a regular file would be, and the warning names both the link and the file it points at. A **dangling** link has no bytes to check, so it is always skipped. **Read the hash as a *freshness* check, not an authorization check:** it proves the file is still the release `init` put there, not that whoever placed the link was entitled to redirect the write — the hashes are of files published in the npm tarball, so anyone who can plant a link in your config tree can plant matching bytes behind it too. (Planting one already implies being able to run code as you, so this is inside the trusted-repo posture — but it is worth knowing what the check does and does not buy.) Every write through a link is reported, naming where the bytes landed. `--uninstall --global` removes the **link** and leaves its **target** where it is, naming the leftover: deleting a file out of a store you manage — typically a git repo — is yours to do, not `init`'s. One consequence to expect: install → uninstall → reinstall does **not** put the stow layout back, because the reinstall writes an ordinary file at the destination and the orphaned copy stays in your store. A **per-project** install does none of this — see below. **The one exception is ModelGuild's own ownership record** (`~/.claude/modelguild/.modelguild-install.json`): only `init` ever writes it, so if that path is a symlink the record is written *through* the link **whatever is behind it** — there is no record *of* the record to check the bytes against, so the ownership rule above has nothing to compare and cannot apply here. `init` warns, naming both the link and the file the bytes landed in, so if you had pointed it at a file of your own you will hear about it — but **the warning is part of the run's report, printed when `init` finishes, not a prompt you can interrupt**, and by the time you read it the file's previous contents are gone. `--uninstall --global` removes the link itself and leaves the file it pointed at where it is, naming it (a link that is *dangling* at uninstall time is left alone entirely) — either way nothing outside your config tree is deleted, and nothing restores what the install overwrote. A **per-project** install is stricter: it refuses a **live** symlink anywhere in a destination path — including a symlinked `.claude/` or `.opencode/` — and names the path it refused. A **dangling** link is not refused there: a dangling payload file is skipped like any other non-file, while a dangling *directory* component fails the install with a raw `ENOENT` from `mkdir` rather than a message about symlinks. You still register the MCP server once, globally — step 3 with `-s user`. Verify with `npx modelguild doctor --global` (checks the global locations). Keep the per-project install if you'd rather scope the payload to specific repos.
