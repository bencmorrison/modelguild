...
- Every def allows what a Claude Code subagent doing that task has. `task` is the one tool denied on all three, and it has a harness reason: a Claude Code subagent has no Agent tool. A tool a future opencode adds is denied by the floor until it is re-allowed here; that is the floor's stated cost, and re-allowing it is the PARITY default.
